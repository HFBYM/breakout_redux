...
public:
	...
	RenderObj(const std::string& tex, const std::string& shader, glm::vec4 color = glm::vec4(1.0f), float rotate = 0.0f) :
		color(color), tex(tex), shader(shader), rotate(rotate), Object("RenderObj") { }

    /// @brief this can let RenderObj have independent instance 
    RenderObj(const std::string& id_name, const glm::vec2& pos, const glm::vec2& size, const std::string& tex, 
		const std::string& shader, glm::vec4 color = glm::vec4(1.0f), float rotate = 0.0f) :Object(pos, size, id_name),
